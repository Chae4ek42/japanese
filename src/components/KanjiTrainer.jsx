...
import { getKanjiInfo, getPracticeWords, getWordsForKanji } from '../data/kanji/bank'
...
export function KanjiTrainer({
  ...
  useEffect(() => {
    ...
    function onPointerDown(event) {
      if (tipRef.current?.contains(event.target)) {
        return
      }
      if (event.target.closest?.('[data-kanji-chip]')) {
        return
      }
      setTip(null)
    }
  ...
  function openKanjiTip(ch, event) {
  ...
  function handleKanjiChipAuxClick(ch, event) {
      ...
      <div className="kanji-trainer-layout">
        <article className="kanji-panel kanji-trainer-hero">
          <div className="kanji-panel-body">
            <button
              ...
              title="Колёсико — полная карточка"
        ...
        <article className="kanji-panel kanji-words-panel">
          <div className="kanji-words-head">
            <div>
              <h3>Слова с этим кандзи</h3>
              <p className="subsection-note">
                {words.length} из {totalAvailable}
                {complexityFilter ? ' · фильтр сложности' : ''}
              </p>
            </div>
            {activeWord ? (
          ...
          {!activeWord ? (
            <div className="kanji-panel-body chart-empty" data-testid="kanji-no-words">
              Нет подходящих слов. Выключите фильтр сложности или выучите больше соседних кандзи.
            </div>
          ) : (
            <>
              <div
                  ...
                  {splitWriting(activeWord.writing).map((ch, index) =>
                    KANJI_CHAR_RE.test(ch) ? (
                      <button
                        ...
                        title="ЛКМ — подсказка · колёсико — карточка"
                ...
                {revealed ? (
                  <div className="kanji-word-details">
                    ...
                    <p className="kanji-word-tags">
                      {activeWord.jlpt ? `JLPT N${activeWord.jlpt}` : 'без JLPT-тега'}
                      {activeWord.common ? ' · частое' : ''}
                    </p>
                  </div>
                ) : (
                  <p className="kanji-word-hint">Пробел — показать или спрятать чтение и перевод</p>
                )}
              </div>
        ...
        >
          <div className="kanji-tip-head">
            ...
            {tip.details ? (
              ...
              <span className="kanji-tip-level is-muted">вне N5–N3</span>
            )}
          </div>
          {tip.details ? (
            <>
              ...
              <div className="kanji-tip-readings">
                ...
                <div className="kanji-reading is-kun">
                  <span className="kanji-reading-label">кун</span>
                  <span title="· отделяет чтение кандзи от окуриганы">
            ...
            <p className="kanji-tip-meanings">Этого знака нет в базе N5–N3.</p>
          )}
          <button
            type="button"
