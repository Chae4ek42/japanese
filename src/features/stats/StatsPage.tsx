import type { NumberMode } from '../../shared/lib/types'
import type { StatsPageProps, MetricCardProps } from '../../shared/lib/component-props'
...
export function StatsPage({ kanaStats, kanaHistory, kanaHyperparams, numbersStats }: StatsPageProps) {
  const now = Date.now()
  ...
  const practicedNumberCards = useMemo(() => {
    return Object.keys(numbersStats)
      ...
      .map((cardId) => {
        ...
        try {
          const card = createNumberCard(value, mode as NumberMode)
  ...
  const kanaProgressItems = [
    {
      ...
      tip: 'Доля верных ответов среди всех попыток, ошибок и подсказок.',
    },
    {
      label: 'Мастерство',
      value: `${kanaGlobal.mastery}%`,
      percent: kanaGlobal.mastery,
      tip: 'Средняя уверенность по всей кане.',
    },
    {
      label: 'Закреплено',
      ...
      tip: 'Знаки с устойчивой серией чистых ответов и высоким мастерством.',
    },
  ]

  return (
    <main className="stats-page">
      <section className="page-surface stats-panel" data-testid="stats-page">
        <div className="section-heading">
          <h2>Прогресс</h2>
        </div>

        <div className="stats-hero" data-testid="stats-hero">
          {kanaProgressItems.map((item) => (
            <article key={item.label} className="stats-hero-metric" data-testid={`stats-hero-${item.label}`}>
              <span>
        ...
        <section className="stats-subsection">
          <div className="stats-chapter-title">
            <h3>Кана</h3>
          </div>
          ...
          <div className="metric-grid">
            ...
            <MetricCard
              ...
              tip="Среднее время от появления знака до верного ответа."
            />
            <MetricCard
              label="Лучшая серия"
              value={kanaGlobal.bestStreak}
              tip="Самая длинная серия верных ответов подряд по одному знаку."
            />
          </div>
          ...
          <div className="stats-chapter-title">
            ...
            <p className="subsection-note">Чем темнее клетка, тем увереннее знак.</p>
          </div>
          ...
          {hardKanaCards.length ? (
            ...
            <div className="chart-empty">Пока нет практики по кане.</div>
          )}
        </section>
        ...
        <section className="stats-subsection" data-testid="numbers-stats">
          ...
          {numbersGlobal ? (
            ...
            <div className="chart-empty">Пока нет практики по числам.</div>
          )}
        </section>
      </section>
    </main>
...
function MetricCard({ label, value, tip }: MetricCardProps) {
  return (
