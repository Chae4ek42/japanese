import type { ActivityChartProps, LatencySparklineProps, MasteryMapProps } from '../../shared/lib/component-props'
...
export function ActivityChart({ daily, days = 14 }: ActivityChartProps) {
  const now = Date.now()
  const items = []
...
export function LatencySparkline({ recent }: LatencySparklineProps) {
  if (recent.length < 2) {
...
export function MasteryMap({ groups, script, statsMap }: MasteryMapProps) {
  return (
          ...
          {group.entries.map((entry) => {
            ...
            return (
              <span
                ...
                style={untouched ? undefined : ({ '--mastery': mastery } as CSSProperties)}
